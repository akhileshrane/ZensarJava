...
public class StudentDao {
	...
	public int addStudent(Student student) throws ClassNotFoundException, SQLException{
		...
		Connection con= DriverManager.getConnection(url,userName,password);
		Statement st=con.createStatement();
		numberOfStudentsAdded=st.executeUpdate("insert into student values(27,'soham',18,'cs');");
		con.close();
		st.close();
	...
	public int updateStudent(Student student)throws ClassNotFoundException, SQLException {
		...
		Connection con= DriverManager.getConnection(url,userName,password);
		Statement st=con.createStatement();
		numberOfStuentsUpdated=st.executeUpdate("UPDATE student SET course='Computers' where course='cs';");
		con.close();
		st.close();
